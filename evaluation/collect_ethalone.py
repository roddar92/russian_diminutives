...
class EthaloneCorporaCollector:
    ...
    def get_possible_dim_engings(name):
        if name.endswith('аша') or name.endswith('уля'):
            ...
            base, endings = name[:-1], ['юша', 'юшка', 'юшечка', 'юшенька', 'юта', 'юха', 'ечка', 'енька']
            ...
            base = name[:-1]
            endings = ['ечка', 'ик', 'енька', 'юля', 'юша', 'ёк']
